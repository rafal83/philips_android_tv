...
def main():
	...
	parser.add_argument("--path",  dest='path', help="Path like 6/channeldb/tv")
	parser.add_argument("--key", dest='key', help="")
	parser.add_argument("--volume", dest='volume', help="")
	parser.add_argument("--pair",  dest='pair', help="Command to pair")
	...
	if args.path:
		...
		if args.key:
			config['body'] = { "key" : args.key }
			post_command(config)
		elif args.volume:
			if args.volume == 0:
				config['body'] = { "muted" : "true", "current": args.volume }
			else:
				config['body'] = { "muted" : "false", "current": args.volume }
 			post_command(config)
		else:
			get_command(config)
